...
class communityranking:
    ...
    def commRanking(self,numTopComms):
        ...
        for rank, rcomms in enumerate(rankedCommunities[:numTopComms]):
            ...
            tmslUsrsCentral, tmslUsrsProfPics, hashtagList, keywordList, bigramList, tmptweetids, commTwText, urlList, topic, tmpkeywrds = [], [], [], [], [], [], [], [], [], []
        ...
        '''
        webDrawFile = open('./Com_Graph/web/jsons/'+self.dataCollection+'communities.json', 'w')
    ...
    def buildDynCommGraphFiles(self, strRank, commUserDict):
        ...
        for name in allUsers:
            if name in userDict:
                allUsernames.append({'screen_name':name,'avatar':self.usernameProfPicDict[name],'id':userDict[name]['id'],
                    'followers_count':userDict[name]['followers_count'],'listed_count':userDict[name]['listed_count'],
                    'friends_count':userDict[name]['friends_count'],'description':userDict[name]['description'],
                    'name':userDict[name]['name'],'location':userDict[name]['location'],
                    'statuses_count':userDict[name]['statuses_count']})
            else:
        ...
        webDrawDataFile = open('./Com_Graph/web/jsons/'+self.dataCollection+'users' + str(int(strRank)+1) +'.json', 'w')
